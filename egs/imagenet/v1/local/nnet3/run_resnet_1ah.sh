...
if [ $stage -le 1 ]; then
  ...
  conv-layer name=conv1 height-in=224 height-out=224 time-offsets=-3,-2,-1,0,1,2,3 required-time-offsets=0 height-offsets=-3,-2,-1,0,1,2,3 num-filters-out=$nf1 param-stddev=0.003
  res-block name=res2 num-filters=$nf1 height=224 time-period=1 $res_opts param-stddev=0.003
  res-block name=res3 num-filters=$nf1 height=224 time-period=1 $res_opts param-stddev=0.003
  conv-layer name=conv4 height-in=224 height-out=112 height-subsample-out=2 time-offsets=-1,0,1 $common num-filters-out=$nf2 param-stddev=0.003
  res-block name=res5 num-filters=$nf2 height=112 time-period=2 $res_opts param-stddev=0.003
  res-block name=res6 num-filters=$nf2 height=112 time-period=2 $res_opts param-stddev=0.003
  conv-layer name=conv7 height-in=112 height-out=56 height-subsample-out=2 time-offsets=-2,0,2 $common num-filters-out=$nf3 param-stddev=0.0015
  res-block name=res8 num-filters=$nf3 num-bottleneck-filters=$nb3 height=56 time-period=4 $res_opts param-stddev=0.0015
  res-block name=res9 num-filters=$nf3 num-bottleneck-filters=$nb3 height=56 time-period=4 $res_opts param-stddev=0.0015
  res-block name=res10 num-filters=$nf3 num-bottleneck-filters=$nb3 height=56 time-period=4 $res_opts param-stddev=0.0015
  channel-average-layer name=channel-average input=Append(0,16,32,48,64,80,96,112,128,144,160,176,192,208) dim=$nf4
...
if [ $stage -le 2 ]; then
  ...
  steps/nnet3/train_raw_dnn.py --stage=$train_stage \
    ...
    --image.augmentation-opts="--horizontal-flip-prob=0.5 --horizontal-shift=0.1 --vertical-shift=0.1 --num-channels=3 \
                               --crop=true --crop-size=224 --crop-scale-min=256 --crop-scale-max=512" \
    --trainer.srand=$srand \
    ...
    --trainer.optimization.proportional-shrink=0.0 \
