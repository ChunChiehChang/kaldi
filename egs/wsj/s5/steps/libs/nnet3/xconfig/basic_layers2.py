...
class XconfigOutputBagOfFeaturesLayer(XconfigLayerBase):
    ...
    def set_default_configs(self):
        ...
        self.config = {'input': '[-1]',
                       ...
                       # L1 Affine layer for bag of features.
                       'affine-transform-file': '',
                       'ng-bof-affine-options': '',
                       'bof-learning-rate-factor': '',
                       'bof-l2-regularize': '',
                       'bof-max-change': 1.5,
                       'bof-input-dim': '',
                       'bof-output-dim': '',
    ...
    def _generate_config(self):
        ...
        bof_options = self.config['ng-bof-affine-options']
        for opt in ['learning-rate-factor', 'l2-regularize', 'max-change']:
            if self.config[opt] != '':
                bof_options += ' {0}={1}'.format(opt, self.config['bof-'+opt])
        ...
        configs.append(('ref', line))
        '''
        line = 'component name={0}.bof-affine type=AffineComponent matrix={1}'.format(
            self.name, transform_file)
        configs.append(('final', line))
        line = 'component name={0}.bof-affine type=AffineComponent input-dim={1} output-dim={2} {3}'.format(
            self.name, self.config['bof-input-dim'], self.config['bof-output-dim'], bof_options)
        configs.append(('ref', line))
        line = 'component-node name={0}.bof-affine component={0}.bof-affine input={1}'.format(
            self.name, cur_node)
        cur_node = '{0}.bof-affine'.format(self.name)
        configs.append(('final', line))
        configs.append(('ref', line))
