...
if [ $stage -le 3 ]; then
  ...
  local/prepare_dict.sh --dir data/local/dict_nosp --lm-dir data/local/lm
...
if [ $stage -le 5 ]; then
  ...
  if [[  $(hostname -f) ==  *.clsp.jhu.edu ]]; then
    ...
    utils/create_split_dir.pl /export/b{02,11,12,13}/$USER/kaldi-data/egs/librispeech/s5b/$mfcc/storage \
     $mfccdir/storage
  fi
fi
